...
const Asignaciones = () => {
    ...
    useEffect(() => {
        setFilteredAsignaciones(
            asignaciones.filter(a =>
                a.bien?.codigoInterno.toLowerCase().includes(historySearch.toLowerCase()) ||
                a.bien?.descripcion.toLowerCase().includes(historySearch.toLowerCase()) ||
                a.ubicacionDestino?.nombre.toLowerCase().includes(historySearch.toLowerCase()) ||
                a.responsableDestino?.nombres.toLowerCase().includes(historySearch.toLowerCase()) ||
                a.responsableDestino?.apellidos.toLowerCase().includes(historySearch.toLowerCase()) ||
                a.motivo.toLowerCase().includes(historySearch.toLowerCase())
            )
        );
    }, [asignaciones, historySearch]);
    ...
    const fetchData = async () => {
        try {
            ...
            setFilteredAsignaciones(asignacionesRes.data);
        ...
        <div>
            ...
            <div className="card">
                <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                    <h2 className="text-xl font-semibold text-gray-900">📋 Historial de Asignaciones</h2>
                ...
                {filteredAsignaciones.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                        {historySearch ? 'No se encontraron asignaciones con ese término' : 'No hay asignaciones registradas'}
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
